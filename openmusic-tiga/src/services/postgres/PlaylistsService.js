...
class PlaylistsService {
  ...
  async addSongActivity(playlistId, songId, userId, action) {
    const time = new Date();
    ...
    if (!result.rows.length) {
      throw new NotFoundError('Activity gagal ditambahkan.');
    }
  }

  async getSongActivity(playlistId) {
    const query = {
  ...
  async deletePlaylistActivity(playlistId) {
    ...
    if (!result.rows.length) {
      throw new NotFoundError('Gagal dihapus dari activity.');
    }
  }
